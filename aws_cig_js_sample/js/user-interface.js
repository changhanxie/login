...
function modalFormEnter(){
	...
	let  registUserName = $("#registUserName").val();
	let  registUserEmail = $("#registUserEmail").val();
	let  registUserPassword = $("#registUserPassword").val();

	console.log(registUserName);
	console.log(registUserEmail);
	console.log(registUserPassword);

	let userNameForReset = $("#userNameForReset").val();
	let newUserPasswordForReset = $("#newUserPasswordForReset").val();
	console.log(userNameForReset);
	console.log(code);
	console.log(newUserPasswordForReset);
	
	let callback;
	let message;
	switch (buttonText){
	case "Sign Up":
		message = `user <i>${registUserName}</i> added to the user pool`;
		callback = createCallback(message, registUserName, registUserEmail, "No", "Created");	
		signUpUser(registUserName, registUserEmail, registUserPassword, callback);
		break;
	...
	case "Request":
		message = `confirmation code for password reset has been sent to <i>${userNameForReset}</i>`;
		callback = createCallback(message, userNameForReset, 
		                          "", "", "Code sent");

		sendPasswordResetCode(userNameForReset, callback);
	...
	case "Reset":
		message = `user <i>${userNameForReset}</i> reset the password`;
		callback = createCallback(message, userNameForReset, 
		                          "", "true", "New password");

		confirmPasswordReset(userNameForReset, code, newUserPasswordForReset, callback);
		$("#modalFormButton").text("Sign In");
		break	
	}
...
function actionSignInUser() {
	updateModal(false, false, false, false, false, "Sign In", "");
}


function actionAfterSignInUser() {
